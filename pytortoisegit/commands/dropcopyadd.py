"""commands/dropcopyadd.py —— /command:dropcopyadd（复制文件）。"""
...
@register("dropcopyadd")
def dropcopyadd(ctx: CommandContext):
    ...
    try:
        for p in paths:
            ...
            def _bg():
                shutil.copy2(p, os.path.join(repo.root, os.path.basename(p)))
                return True
            dlg.run(_bg)
            dlg.exec()
