...
class MergeFrm(QMainWindow):
    ...
    def _build_statusbar(self):
        ...
        self._col_lab = QLabel(tr("tm_col", "Column: 0"))
    ...
    def _create_actions(self):
        ...
        a: dict[str, QAction] = {}
        a["open"] = self._act(tr("tm_open", "Open"), self._file_open, "Ctrl+O", icon="IDI_OPEN")
        a["save"] = self._act(tr("tm_save", "Save"), self._save_result, "Ctrl+S", icon="IDI_SAVE")
        a["saveas"] = self._act(tr("tm_saveas", "Save As"), self._save_as, icon="IDI_SAVEAS")
        a["patch"] = self._act(tr("tm_patch", "Create Patch File"), self._create_patch, icon="IDI_TORTOISEUDIFF")
        a["filelist"] = self._act(tr("tm_filelist", "Show/Hide Patch File List"), self._show_filelist)
        a["settings"] = self._act(tr("tm_settings", "Settings"), self._settings, icon="IDI_GENERAL")
        a["about"] = self._act(tr("tm_about", "About TortoiseGitMerge..."), self._about)
        a["exit"] = self._act(tr("tm_exit", "Exit"), self.close, "Ctrl+Q")
        a["reload"] = self._act(tr("tm_reload", "Reload"), self._load, "F5", icon="IDI_REFRESH")
        a["undo"] = self._act(tr("tm_undo", "Undo"), self._undo, "Ctrl+Z", icon="IDI_RESET")
        a["redo"] = self._act(tr("tm_redo", "Redo"), self._redo, "Ctrl+Y", icon="IDI_RESTORE")
        self._act_edit_en = a["enable_edit"] = self._act(
            tr("tm_enable_edit", "Allow Editing"), self._toggle_edit, checkable=True, icon="IDI_NOTEPAD")
        a["copy"] = self._act(tr("tm_copy", "Copy"), self._copy, "Ctrl+C", icon="IDI_BLAME_POPUP_COPY")
        a["paste"] = self._act(tr("tm_paste", "Paste"), self._paste, "Ctrl+V")
        a["find"] = self._act(tr("tm_find", "Find"), self._find, "Ctrl+F", icon="IDI_FILTEREDIT")
        a["find_next"] = self._act(tr("tm_find_next", "Find Next"), self._find_next, "F3")
        a["find_prev"] = self._act(tr("tm_find_prev", "Find Previous"), self._find_prev, "Shift+F3")
        a["goto"] = self._act(tr("tm_goto", "Go to Line"), self._goto_line, "Ctrl+G")
        a["mark"] = self._act(tr("tm_mark", "Mark as Resolved"), self._mark_resolved, icon="IDI_MERGEACTIVE")
        a["prev_diff"] = self._act(tr("tm_prev_diff", "Previous Difference"),
                                   lambda: self._goto_diff(-1), "Ctrl+Up", icon="IDI_JUMPUP")
        a["next_diff"] = self._act(tr("tm_next_diff", "Next Difference"),
                                   lambda: self._goto_diff(1), "Ctrl+Down", icon="IDI_JUMPDOWN")
        a["prev_conf"] = self._act(tr("tm_prev_conf", "Previous Conflict"),
                                   lambda: self._goto_conflict(-1), icon="IDI_ACTIONCONFLICTED")
        a["next_conf"] = self._act(tr("tm_next_conf", "Next Conflict"),
                                   lambda: self._goto_conflict(1), icon="IDI_CONFLICTEDLINE")
        a["prev_inline"] = self._act(tr("tm_prev_inline", "Previous Inline Difference"),
                                     lambda: self._goto_inline(-1))
        a["next_inline"] = self._act(tr("tm_next_inline", "Next Inline Difference"),
                                     lambda: self._goto_inline(1))
        a["use_left_block"] = self._act(tr("tm_use_left_block", "Use Left Block"),
                                        self._on_use_left_block)
        a["use_left_file"] = self._act(tr("tm_use_left_file", "Use Left File"),
                                       self._on_use_left_file)
        a["use_left_before"] = self._act(tr("tm_use_left_before", "Use Left Then Right"),
                                         self._on_use_both_left_first)
        a["use_right_before"] = self._act(tr("tm_use_right_before", "Use Right Then Left"),
                                          self._on_use_both_right_first)
        a["use_theirs"] = self._act(tr("tm_use_theirs", "Use Left Text Block"),
                                    self._on_use_theirs)
        a["use_mine"] = self._act(tr("tm_use_mine", "Use Right Text Block"),
                                  self._on_use_mine)
        a["use_theirs_then"] = self._act(tr("tm_use_theirs_then", "Use Left Then Right Text Block"),
                                         self._on_use_theirs_then)
        a["use_mine_then"] = self._act(tr("tm_use_mine_then", "Use Right Then Left Text Block"),
                                       self._on_use_mine_then)
        self._act_show_ws = a["show_ws"] = self._act(
            tr("tm_show_ws", "Show Whitespace"), self._toggle_show_ws, checkable=True)
        ws_group = QActionGroup(self)
        ws_group.setExclusive(True)
        self._act_cmp_ws = a["cmp_ws"] = self._act(
            tr("tm_cmp_ws", "Compare Whitespace"),
            lambda on: on and self._set_ws(IgnoreWS.None_), checkable=True, checked=True)
        self._act_ign_ws = a["ign_ws"] = self._act(
            tr("tm_ign_ws", "Ignore Whitespace Changes"),
            lambda on: on and self._set_ws(IgnoreWS.WhiteSpaces), checkable=True)
        self._act_ign_all_ws = a["ign_all_ws"] = self._act(
            tr("tm_ign_all_ws", "Ignore All Whitespace Changes"),
        ...
        self._act_inline = a["inline"] = self._act(
            tr("tm_inline", "Inline Differences"), self._toggle_inline, checkable=True, checked=True)
        self._act_inline_w = a["inline_word"] = self._act(
            tr("tm_inline_word", "Word-level Inline Differences"), self._toggle_inline_word,
            checkable=True, checked=True)
        a["regex"] = self._act(tr("tm_regex", "Regex Filters"), self._regex_filter, icon="IDI_FILTEREDIT")
        self._act_ign_cmt = a["ignore_comments"] = self._act(
            tr("tm_ignore_comments", "Ignore Comments"), self._toggle_ignore_comments, checkable=True)
        self._act_ign_cmt2 = self._act_ign_cmt
        self._act_ign_eol = a["ignore_eol"] = self._act(
            tr("tm_ignore_eol", "Ignore Line Endings"), self._toggle_ignore_eol,
            checkable=True, checked=True)
        a["view_bars"] = self._act(tr("tm_view_bars", "Bars"), lambda: None)
        self._act_ldb = a["linediffbar"] = self._act(
            tr("tm_linediffbar", "Line Diff Bar"), self._toggle_linediff, checkable=True, checked=True)
        self._act_loc = a["locator"] = self._act(
            tr("tm_locator", "Locator Bar"), self._toggle_locator, checkable=True, checked=True)
        self._act_sb = a["statusbar"] = self._act(
            tr("tm_statusbar", "Status Bar"), self._toggle_statusbar, checkable=True, checked=True)
        self._act_wrap = a["wrap"] = self._act(
            tr("tm_wrap", "Wrap Lines"), self._toggle_wrap, checkable=True)
        self._act_oneway = a["oneway"] = self._act(
            tr("tm_oneway", "Toggle One/Two Panes"), self._toggle_oneway, checkable=True)
        a["switch"] = self._act(tr("tm_switch", "Swap Left/Right Views"), self._switch_left,
                                icon="IDI_SWITCHLEFTRIGHT")
        self._act_collapse = a["collapse"] = self._act(
            tr("tm_collapse", "Collapse"), self._toggle_collapse, checkable=True)
        a["help"] = self._act(tr("tm_help_topics", "Help Topics"), self._help, "F1")
        self._act_moved = self._act(tr("tm_moved", "Moved Blocks"), self._toggle_moved,
                                    checkable=True, checked=True)
    ...
    def _load(self):
        ...
        if self._local_left and self._local_right:
            ...
            self._header.setText(
                f" {self.path}  ·  ours={self.rev2 or tr('diff_working_tree', 'Working tree')}  theirs={self.rev1 or tr('diff_working_tree', 'Working tree')}   "
                f"{tr('tm_conflicts', 'Conflict')} {conflicts}")
        ...
        self._header.setText(
            ...
            f"{(self.rev1 or self._local_left or tr('diff_working_tree', 'Working tree'))} → "
            f"{(self.rev2 or self._local_right or tr('diff_working_tree', 'Working tree'))}   +{added}/-{removed}")
        self._refresh_linebar()
        self._update_command_ui()
    ...
    def _on_view_line(self, line: int):
        ...
        self._col_lab.setText(tr("tm_col_n", "Column: {}").format(col))
        self._update_locator_viewport()
    ...
    def _conflicts_wont_keep(self) -> bool:
        ...
        box.setText(tr("tm_has_conflicts", "File still has unresolved conflicts (around line {}).").format(idx + 1))
        keep = box.addButton(tr("tm_save_anyway", "Save Anyway"),
                             QMessageBox.ButtonRole.AcceptRole)
        goto = box.addButton(tr("tm_goto_conflict", "Go to Conflict"),
    ...
    def _mark_as_resolved_git(self) -> bool:
        ...
        if self.repo is None or not self.path:
            QMessageBox.warning(self, tr("tm_mark", "Mark as Resolved"),
                                tr("tm_mark_no_repo", "No repository path; cannot git add."))
            return False
        if not mark_resolved(self.repo, self.path):
            QMessageBox.warning(self, tr("tm_mark", "Mark as Resolved"),
                                tr("tm_mark_fail", "git add failed; could not mark as resolved."))
            return False
    ...
    def _mark_resolved(self):
        if self.bottom_view is None:
            QMessageBox.information(self, tr("tm_mark", "Mark as Resolved"),
                                    tr("tm_mark_need_merge", "Only a three-way merge result can be marked as resolved."))
            return
        ...
        if self._mark_as_resolved_git():
            QMessageBox.information(self, tr("tm_mark", "Mark as Resolved"),
                                    tr("tm_mark_ok", "Saved and git added; conflict marked as resolved."))
        self._update_header()
        ...
        if dlg.your_file and (dlg.their_file or dlg.base_file):
            ...
            if self.three_way:
                QMessageBox.information(
                    self, tr("tm_open", "Open"),
                    tr("tm_open_two", "Comparing two files left/right; use the merge command for three files."))
                self.three_way = False
            self._load()
    ...
    def _help(self):
        QMessageBox.information(self, tr("tm_help_topics", "Help Topics"),
                                tr("tm_help_text", "TortoiseGitMerge text compare / merge."))
    ...
    def _create_patch(self):
        if self.repo is None or not self.path:
            QMessageBox.information(self, tr("tm_patch", "Create Patch"),
                                    tr("tm_patch_none", "No repository files to export."))
            return
        path, _ = QFileDialog.getSaveFileName(
            self, tr("tm_patch", "Create Patch File"), "", "Patch (*.patch *.diff)")
        if not path:
            return
        args = ["diff", "--no-color"]
        ...
        with open(path, "w", encoding="utf-8", newline="\n") as fh:
            fh.write(text)
        QMessageBox.information(self, tr("tm_patch", "Create Patch"), path)
    ...
    def _update_header(self):
        ...
        self._header.setText(
            f" {self.path or ''}  ·  {(self.rev1 or tr('diff_working_tree', 'Working tree'))} → {(self.rev2 or tr('diff_working_tree', 'Working tree'))}   "
            f"+{added}/-{removed}   {tr('tm_resolved', 'Resolved')} {solved}")
    ...
    def _save_view_as(self, view: BaseView) -> bool:
        path, _ = QFileDialog.getSaveFileName(
            self, tr("tm_saveas", "Save As"), self._view_save_path(view) or self.path or "")
        if not path:
            return False
    ...
    def _ask_save_which(self, left_mod: bool, right_mod: bool) -> str:
        ...
        box.setWindowTitle(tr("tm_save", "Save"))
        box.setText(tr("tm_save_which", "Multiple panes are writable; which side to save?"))
        btn_left = box.addButton(tr("tm_save_left", "Save Left"),
                                 QMessageBox.ButtonRole.ActionRole)
        btn_right = box.addButton(tr("tm_save_right", "Save Right"),
                                  QMessageBox.ButtonRole.ActionRole)
        btn_all = box.addButton(tr("tm_save_all", "Save All"),
                                QMessageBox.ButtonRole.ActionRole)
    ...
    def _save_merged(self, check_resolved: bool = True) -> bool:
        ...
        if (check_resolved and self.bottom_view is not None
            ...
            ask = QMessageBox.question(
                self, tr("tm_mark", "Mark as Resolved"),
                tr("tm_ask_mark", "Conflict resolved. git add to mark the file?"),
    ...
    def _save_result(self):
        ...
        if len(writable) > 1:
            if len(modified) == 1:
                ...
                if ok:
                    QMessageBox.information(self, tr("tm_save", "Save"),
                                            tr("tm_saved", "Wrote {}").format(
            ...
            if modified or writable:
                ...
                if ok:
                    QMessageBox.information(
                        self, tr("tm_save", "Save"),
                        tr("tm_saved_ok", "Saved."))
                return
        if self._save_merged():
            dest = self._view_save_path(self._target_view())
            QMessageBox.information(self, tr("tm_save", "Save"),
                                    tr("tm_saved", "Wrote {}").format(dest))
